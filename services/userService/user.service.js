  ...
  actions: {
    register: {
      ...
      async handler(ctx) {
        try {
          ...
          const existingEmail = await ctx.call('users.find', { query: { email } });
          if (existingEmail.length > 0) {
            throw new MoleculerError('Email already exists', 400); 
          }

          // Check if the username already exists
          const existingUsername = await ctx.call('users.find', { query: { username } });
          if (existingUsername.length > 0) {
            throw new MoleculerError('Username already exists', 400);
          }
          ...
          // Create the new user
          await ctx.call('users.create', { username, email, password: hashedPassword });

          // Fetch user data without password
          const userDataWithoutPassword = await ctx.call('users.find', { query: { email }, fields: ['username', 'email'] });

          return { message: 'User registered successfully', user: userDataWithoutPassword[0] };
        } catch (error) {
          ...
          throw new MoleculerError('Registration failed', 500, 'REGISTRATION_FAILED', { error });
        }
      }
    }
    ,
    ...
    login: {
      ...
      async handler(ctx) {
        try {
          ...
          // Find users by email
          const users = await ctx.call('users.find', { query: { email } });
          ...
          const token = jwt.sign({ userId: validUser.email }, this.settings.JWT_SECRET);
          ...
          throw new MoleculerError('Login failed', 500, 'LOGIN_FAILED', { error });
        }
      }
    }
    ...
    getUserByToken: {
      async handler(ctx) {
        ...
        try {
          const decodedToken = jwt.verify(token, this.settings.JWT_SECRET);
          ...
          if (error.name === 'JsonWebTokenError') {
            throw new MoleculerError('Invalid token', 401, 'INVALID_TOKEN', { error });
          } else {
            throw new MoleculerError('Error authenticating user', 500, 'AUTH_ERROR', { error });
          }
        }
      }
    }
    ...
    getAllUsers: {
      async handler(ctx) {
        const users = await ctx.call('users.find', { fields: ['username', 'email'] });
        return { users };
      }
    }
    ,
